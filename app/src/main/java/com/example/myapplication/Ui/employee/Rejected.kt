...
class Rejected : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var recyclerView: RecyclerView = findViewById(R.id.recyclerView2)
        ...
        service.getUserInfoByUser("$a","rejected").enqueue(object: Callback<GetResponse> {
            override fun onResponse(call: Call<GetResponse>, response: Response<GetResponse>) {
                recyclerView.adapter =
                    response.body()?.let { ApplicationAdapter(this@Rejected, it.application) }
            }
